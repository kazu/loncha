...
func runBnech(b *testing.B, m list_head.MapGetSet, concurretRoutine, operationCnt int, pctWrites uint64) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		if pctWrites*mc/100 != pctWrites*(mc-1)/100 {
			...
			for pb.Next() {
				_, _ = m.Get(fmt.Sprintf("%d", index&mask))
				index = index + 1
			}
		}
	})
...
func Benchmark_HMap(b *testing.B) {
	...
	benchmarks := []struct {
		...
		// {"HMap               ", 100, 100000, 0, 0x200, list_head.LenearSearchForBucket, list_head.NewHMap()},
	...
	for _, bm := range benchmarks {
		b.Run(fmt.Sprintf("%s w/%2d bucket=%3d", bm.name, bm.percent, bm.buckets), func(b *testing.B) {
			if hmap, ok := bm.mapInf.(*list_head.HMap); ok {
				list_head.MaxPefBucket(bm.buckets)(hmap)
				list_head.BucketMode(bm.mode)(hmap)
			}
...
func Benchmark_Map(b *testing.B) {
	...
	benchmarks := []struct {
		...
		{"HMap               ", 100, 1000000, 0, list_head.NewHMap()},
